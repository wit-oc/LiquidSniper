...
const artifactRoot = path.join(strategyRoot, 'artifacts/v7_system_a_displacement_candidate');
...
const manifestPath = path.join(artifactRoot, 'tv_displacement_candidate_runs.json');
...
const profiles = [
  {
    ...
    label: 'Baseline',
    title: 'Baseline',
    description: 'Clean System A foundation with risk/ATR/range telemetry.',
  },
  {
    id: 'risk-damp-150bps',
    ...
    description: 'Prior drawdown overlay: halve risk above 150 bps stop distance.',
  },
  {
    ...
    description: 'Prior best broad quality filter: require Strength slope in the trade direction.',
  },
  {
    id: 'displacement-quality',
    variant: 'candidate_displacement_quality',
    label: 'Displacement Quality',
    title: 'Displacement Quality',
    description: 'Candidate: Directional Strength Slope + entry range/ATR gate + risk damp overlay.',
  },
  {
    id: 'displacement-atr-regime',
    variant: 'candidate_displacement_atr_regime',
    label: 'Displacement ATR Regime',
    title: 'Displacement ATR Regime',
    description: 'Candidate plus ATR regime damp/veto using the prior damaging ATR buckets.',
  },
];
...
function patchTemplate(template, profile) {
  const title = `Unity UTM Strategy v7 Displacement Test ${profile.title}`;
  const logic = `unity-utm-v7-displacement-test-${profile.id}`;
...
const runs = profiles.map((profile) => {
  const runId = `v7-system-a-displacement-${profile.id}`;
  ...
  return {
    ...
    scriptTitle: `Unity UTM Strategy v7 Displacement Test ${profile.title}`,
    expectedStrategyTitle: `Unity UTM Strategy v7 Displacement Test ${profile.title}`,
...
const manifest = {
  ...
  defaults: {
    ...
    headless: false,
    artifactsDir: 'tradingview/strategy/artifacts/v7_system_a_displacement_candidate/tradingview/automation',
