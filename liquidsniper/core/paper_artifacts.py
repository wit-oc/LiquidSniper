...
def build_run_artifact(proposal: dict[str, Any], execution_result: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "score_total_raw": _f(merged.get("score_total_raw")),
        "score_total_adj": _f(merged.get("score_total_adj") or merged.get("score_total") or merged.get("final_score")),
        "score_total": _f(merged.get("score_total") or merged.get("score_total_adj") or merged.get("final_score")),
        ...
        "gate_trail": (merged.get("gate_checks") or {}).get("gate_trail") if isinstance((merged.get("gate_checks") or {}).get("gate_trail"), list) else [],
        "bias_snapshot": merged.get("bias_snapshot") if isinstance(merged.get("bias_snapshot"), dict) else {},
        "sr_context": merged.get("sr_context") if isinstance(merged.get("sr_context"), dict) else {},
        "policy_snapshot": merged.get("policy_snapshot") if isinstance(merged.get("policy_snapshot"), dict) else {},
        "position_state_before": merged.get("position_state_before") if isinstance(merged.get("position_state_before"), dict) else {},
        "position_state_after": merged.get("position_state_after") if isinstance(merged.get("position_state_after"), dict) else {},
        ...
        "close_confirm_attempts": int(merged.get("close_confirm_attempts") or 0),
        "close_confirm_elapsed_sec": int(merged.get("close_confirm_elapsed_sec") or 0),
        "target_open_ts": merged.get("target_open_ts"),
        "target_close_ts": merged.get("target_close_ts"),
        "matched_candle_open_ts": merged.get("matched_candle_open_ts"),
        "exchange_offset_ms": int(merged.get("exchange_offset_ms") or 0),
        "data_fetch_attempts": int(merged.get("data_fetch_attempts") or 0),
        "breakout_regime": bool(merged.get("breakout_regime")),
        "htf_chop_mode": merged.get("htf_chop_mode") or "soft_hard",
        "htf_chop_threshold_effective": _f(merged.get("htf_chop_threshold_effective")),
        "htf_chop_ci": _f(merged.get("htf_chop_ci")),
        "htf_chop_er": _f(merged.get("htf_chop_er")),
        "htf_chop_norm": _f(merged.get("htf_chop_norm") or merged.get("htf_chop")),
        "htf_chop_soft_max": _f(merged.get("htf_chop_soft_max")),
        "htf_chop_hard_max": _f(merged.get("htf_chop_hard_max")),
        "htf_chop_penalty": _f(merged.get("htf_chop_penalty")),
        "sr_retest_mode": merged.get("sr_retest_mode") or "strict",
        "sr_near_retest_used": bool(merged.get("sr_near_retest_used")),
        "sr_penalty": _f(merged.get("sr_penalty")),
