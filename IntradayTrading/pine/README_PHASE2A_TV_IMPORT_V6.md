...
- 2026-03-07 v6.2.5 finalize branch: switched cluster finalize from symmetric recentering to bound-preserving seed-bound finalize (pad-first, cap-trim with side-biased trimming), added finalize cap controls (`finalizeCapMode`, `finalizeFlipNoShrinkGuard`), and compacted probe diagnostics with multiline row output (`Δmid`, `Δw`) plus optional export-cfg rows (`showExportCfg`) and compact table mode (`debugCompactMode`).
- 2026-03-07 v6.2.5 width-governance patch: made width pass use mode-consistent `widthLimit` (strict/soft/off), added flip-qualified width grace (`flipWidthGracePct/ATR`), added merge-time span caps (`mergeMaxSpanPct/ATR`) to prevent oversized cluster absorption, and expanded probe width diagnostics (`W fin/cap/soft/lim`, guard-hit count, span rejects).
